    ...
    def _get_transactions(
        ...
        if start_date and end_date:
            buy_query = buy_query.filter(
                OrderTransaction.transaction_date >= start_date,
                OrderTransaction.transaction_date <= end_date
            )
        ...
        if start_date and end_date:
            sell_query = sell_query.filter(
                SoldOrder.created_at >= start_date,
                SoldOrder.created_at <= end_date
            )

        sell_records = sell_query.order_by(SoldOrder.created_at.desc()).all()
